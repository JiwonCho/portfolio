...
export const projects: Project[] = [
  ...
  {
    slug: 'privia-promotion-builder-refactor',
    no: '17',
    title: '프로모션 빌더 리팩토링 — 레거시 에디터를 블록 트리로',
    tagline: 'DOM 이 정본이던 4,625줄 단일 파일 에디터를, JSON 블록 트리가 정본인 구조로 다시 세움',
    brand: 'tooling',
    repo: 'pb-refactor',
    tags: ['architecture', 'platform', 'quality'],
    stack: ['Next.js 15', 'React 19', 'TypeScript', 'Zustand', 'Zod', 'Tailwind CSS 4', 'MongoDB', 'dnd-kit'],
    featured: true,
    background:
      '프리비아 기획전을 만드는 사내 프로모션 빌더는 2015~2016년에 만들어진 Express 4 · Jade · jQuery 1.11 기반 도구입니다. 에디터 전부가 edit.js 한 파일 4,625줄에 들어 있고, 테이블 병합 로직이 또 1,384줄을 차지합니다. 무엇보다 편집 중인 미리보기 DOM 자체가 정본이라, 저장은 그 innerHTML 을 문자열로 떠서 보내고 편집 흔적을 손으로 지우는 방식이었습니다. Next.js 15 · TypeScript · Node 22 로 다시 만들면서 데이터 모델부터 뒤집었습니다.',
    implementation: [
      {
        title: '정본을 DOM 에서 JSON 블록 트리로 옮겼습니다',
        body: '레거시는 「미리보기 DOM → DOM 조작 → innerHTML → 저장」이었습니다. 신규는 「블록 트리 → 순수 함수 → JSON 저장」이고, 배포 HTML 은 같은 트리를 renderPromotionHtml() 로 렌더한 결과입니다. 미리보기와 배포 HTML 이 같은 함수에서 나오므로 둘이 어긋날 수 없습니다.',
      },
      {
        title: '블록 참조를 DOM 인덱스에서 id 로 바꿨습니다',
        body: '레거시는 구조 패널과 미리보기가 별개의 DOM 트리인데 .eq(dep1).find().eq(dep2) 같은 인덱스 좌표로 서로를 참조했습니다. 중간에 블록 하나가 추가되면 좌표가 통째로 밀립니다. 판별 유니온 타입의 블록마다 id 를 부여해 참조를 안정화했습니다.',
      },
      {
        title: '레거시 문서를 일괄 마이그레이션 없이 흡수합니다',
        body: '기존 DB 의 promotionList HTML 을 파싱해 블록으로 되돌리고, 저장할 때 정본(pbBlocks)과 렌더 결과(html)를 함께 갱신합니다. 사람이 여는 문서부터 하나씩 구조화되고, 그동안 기존 도구도 같은 DB 를 계속 읽을 수 있습니다. 큰 전환 작업을 한 번에 하지 않아도 되는 경로를 택했습니다.',
      },
      {
        title: '없던 안전장치를 넣었습니다',
        body: '모든 쓰기 경로에 zod 검증을 걸고, 마지막 저장이 무조건 이기던 동시 저장을 낙관적 잠금(409)으로 바꿨습니다. 대상 문서를 Referer 헤더에서 뽑아 쓰던 것을 경로 파라미터로 옮기고, Ctrl+Z / Ctrl+Shift+Z 되돌리기를 추가했습니다.',
      },
      {
        title: '테이블 병합을 순수 도메인 함수로 분리',
        body: '행/열 추가·삭제·병합을 다루던 tableGrid.js 1,384줄을 React·Next 의존이 없는 domain/table-ops.ts 로 옮기고 단위 테스트로 고정했습니다. 도메인 레이어 전체가 프레임워크를 모르기 때문에 렌더러가 바뀌어도 그대로 씁니다.',
      },
      {
        title: '화면은 일부러 그대로 뒀습니다',
        body: '레거시 style.css(79KB)와 이미지·폰트를 원본 그대로 옮기고 마크업 구조, 유닛/템플릿 체계, 클래스명, 커스텀 색상 태그, 편집모드 스위치와 우측 슬라이드 패널, 「HTML 복사 → 어드민 붙여넣기」 플로우를 유지했습니다. 쓰던 사람이 재학습 없이 그대로 쓸 수 있어야 교체가 가능하기 때문입니다.',
      },
      {
        title: '구형 DB 드라이버가 만든 빌드 불안정을 잡았습니다',
        body: '운영 MongoDB 가 3.0.2 라 최신 드라이버로는 접속이 안 돼 mongodb@3.7 을 씁니다. 동적 require 가 많은 구형 CJS 패키지라 번들에 들어가면 재컴파일마다 모듈 그래프가 흔들려 clientReferenceManifest·ChunkLoadError 가 산발적으로 났습니다. serverExternalPackages 로 번들에서 빼내 같은 조건 재현 시 10회 중 2회 실패 → 30회 중 0회로 정리했습니다.',
      },
      {
        title: '판단 근거를 문서로 남겼습니다',
        body: '레거시 분석, 새 아키텍처, 데이터 소스 전략, 우측 패널 기능 대조표, 도움말 기준 구현 점검표, AS-IS/TO-BE 대조 도구, 인수인계까지 8종 2,474줄. 무엇을 유지하고 무엇을 왜 바꿨는지, 의도적으로 남긴 차이가 무엇인지를 다음 사람이 읽을 수 있게 정리했습니다.',
      },
    ],
    metrics: [
      { value: '4,625줄', label: '단일 파일 에디터를 대체' },
      { value: '65개', label: '소스 파일 · 13,456줄' },
      { value: '30건', label: '테스트' },
      { value: '2,474줄', label: '설계·인수인계 문서 8종' },
    ],
  },
  {
    slug: 'promotion-publishing-generator',
    no: '18',
    title: '프로모션 퍼블 자동 생성기',
    tagline: '이미지 URL 과 좌표만 넣으면 PC·모바일 프로모션 마크업이 나오는 사내 도구를 단독 제작',
    brand: 'tooling',
    repo: 'pub-maker',
    tags: ['platform', 'quality'],
    stack: ['Vanilla JS', 'Vite 5', 'jsdom', 'Prettier', 'Vercel'],
    featured: true,
    background:
      '프로모션 페이지 퍼블리싱은 이미지 위에 %좌표로 클릭 영역을 얹는 반복 작업입니다. 브랜드마다 마크업 관례가 달라 세 갈래(투어비스 항공일반 · 베네피아/휴가샵 · SKT)로 나뉘고, 매번 손으로 좌표를 계산해 HTML 을 적었습니다. 입력만 받아 코드를 뱉는 생성기를 만들어 배포했습니다.',
    implementation: [
      {
        title: '3중 복사를 막는 공통 모듈을 먼저 뽑았습니다',
        body: '브랜드별 생성기 두 개가 이미 90% 중복 상태였습니다. 새 기능을 그대로 넣으면 세 벌이 됩니다. 순수 함수 위주의 PubFeatures 네임스페이스 하나만 노출하는 features.js 를 만들어 좌표 변환·마크업 생성·설정 패널 렌더러를 단일 소스로 유지했습니다.',
      },
      {
        title: '레퍼런스 마크업을 먼저 분석해 계획서를 썼습니다',
        body: '실제 운영 기획전 3건의 HTML 을 뜯어 탭 스크롤·스티키 탭·영상 첨부가 어떤 마크업으로 되어 있는지 정리하고, 그 위에서 설계 방향과 API 표, 구현 순서, 검증 체크리스트, 이번 범위에서 제외할 것까지 338줄 계획서로 결론을 내고 착수했습니다.',
      },
      {
        title: '탭 스크롤 — 반응형에서 흔들리지 않는 앵커',
        body: '섹션 마커를 가로 전체 폭 · 높이 1px 로 출력해 이미지 크기가 뷰포트에 따라 달라져도 앵커 위치가 밀리지 않게 했습니다. 탭 N개면 섹션 마커 N개와 섹션마다 탭 메뉴 한 벌을 함께 생성합니다.',
      },
      {
        title: '스티키 탭 — 파일명 규칙으로 활성 상태 전환',
        body: '스크롤에 따라 탭바를 상단 고정하고 현재 섹션에 맞춰 탭 이미지를 _off ↔ _on 으로 바꿉니다. 파일명 끝만 안전하게 치환하는 함수를 따로 둬, 경로 중간에 같은 문자열이 있어도 오작동하지 않게 했습니다.',
      },
      {
        title: '영상 첨부 — 두 소스를 같은 오버레이로',
        body: '유튜브 URL(watch · youtu.be · embed · shorts)과 11자 ID 를 모두 받아 정규화하고, mp4 는 video 태그로, 유튜브는 iframe 으로 같은 %좌표 오버레이에 배치합니다.',
      },
      {
        title: 'UI 배선까지 테스트로 고정',
        body: '코드 생성 함수의 단위 테스트와 별개로, jsdom 으로 index.html 을 통째로 올려 입력 → 미리보기 → 코드 출력 배선이 실제로 이어지는지 검증합니다(1,158줄). 생성기는 결과물이 문자열이라 눈으로만 확인하면 회귀를 놓치기 쉽습니다.',
      },
      {
        title: '쓰는 사람을 위한 문서를 함께 만들었습니다',
        body: '사용설명서 750줄(조작법 + 기능 점검 체크리스트), PDF 배포용 가이드 원고 569줄, 원고에 넣을 화면 12장의 촬영 방법까지 정리했습니다. 실무자가 문의 없이 쓸 수 있는 상태로 넘기는 것이 목표였습니다.',
      },
      {
        title: '번들하지 않는 선택',
        body: 'js/*.js 는 전역 스코프를 공유하는 일반 스크립트라 Vite 가 번들하지 않습니다. 빌드 시 그대로 복사하는 플러그인을 두어 스크립트 로드 순서를 유지했습니다. 생성기 특성상 산출 코드가 그대로 복사돼 나가야 해서 모듈 변환이 오히려 방해가 됩니다.',
      },
    ],
    metrics: [
      { value: '3종', label: '브랜드별 생성기 (투어비스 · 베네피아/휴가샵 · SKT)' },
      { value: '4,790줄', label: '생성기 코드' },
      { value: '1,367줄', label: '테스트 (jsdom UI 배선 포함)' },
      { value: '1,834줄', label: '사용설명서 · 가이드 원고 · 설계 문서' },
    ],
    links: [
      { label: '라이브 데모', href: 'https://pub-maker-drab.vercel.app/', type: 'live' },
      { label: 'GitHub', href: 'https://github.com/JiwonCho/promotionBuilder', type: 'repo' },
    ],
  },
  {
